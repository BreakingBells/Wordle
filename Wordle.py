...
WORDS = ["aback","abase","abate","abbey","abbot","abhor","abide","abled","abode","abort",
...
GRAY   = "\033[100m\033[30m"  # dark gray background
RESET  = "\033[0m"

# Track keyboard letter states
letter_status = {c: "unused" for c in "ABCDEFGHIJKLMNOPQRSTUVWXYZ"}

def update_letter_status(guess, answer):
...
def print_keyboard():
    """Print Wordle-style colored keyboard."""
...
def get_feedback(guess, answer):
    """Return colored feedback for a guess."""
    result = [""] * len(guess)
    ...
    for i, ch in enumerate(guess):
        ...
        for j, a_ch in enumerate(answer):
            if not used[j] and ch == a_ch:
                ...
                break
        if found_yellow:
            result[i] = YELLOW + ch.upper() + RESET
        else:
            result[i] = ch.upper()
...
def play_wordle():
    ...
    print(f"Guess the {word_length}-letter word. You have {attempts} tries.\n")
    ...
    for attempt in range(1, attempts + 1):
        ...
        update_letter_status(guess, answer)

        if guess == answer:
            print("\nYou got it!")
            return

    print(f"\nOut of attempts! The word was: {answer.upper()}")
